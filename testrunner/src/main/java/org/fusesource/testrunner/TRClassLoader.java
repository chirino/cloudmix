...
/**
 * This classloader can be used to dynamically load classes at runtime.
 * Typically applications using TestRunner will not use TRClassloader because
 * their application specific classes are already in their classpath.
 * TestRunner, however, must make use of TRClassLoader because it does not
 * 'know' what classes may be passed to it.
 */
public class TRClassLoader extends ClassLoader {
    ...
    /**
     * Constructs a new classloader with the specified classpath ( + those
     * already loaded by the primordial classloader).
     * 
     * @param classpath
     *            The classpath
     * @exception java.lang.Exception
     */
    public TRClassLoader(String classpath) throws Exception {
        super();
        delegate = this.getClass().getClassLoader();
        ...
        while (cpTokenizer.hasMoreTokens()) {
            if (DEBUG)
                System.out.println("Dynamically loading: " + classpath);
            ...
            //and load those classes
                ...
                while ((entry = zipStream.getNextEntry()) != null) {
                    if (DEBUG)
                        System.out.print("Compression for " + entry.getName() + ":");
                    if (entry.getMethod() == ZipEntry.STORED) {
                        if (DEBUG)
                            System.out.println("STORED: " + entry.getCompressedSize() + "/" + entry.getSize());
                    } else if (entry.getMethod() == ZipEntry.DEFLATED) {
                        if (DEBUG)
                            System.out.println("DEFLATED: " + entry.getCompressedSize() + "/" + entry.getSize());
                    } else {
                        if (DEBUG)
                            System.out.println("Compression unknown " + entry.getCompressedSize() + "/" + entry.getSize());
                    }
                    ...
                    try {
                        ...
                        if (DEBUG)
                            System.out.println("Read " + rb + " for " + entry.getName() + "into array of size " + entryBytes.length);
    ...
    private void storeClass(String name, byte[] bytes) throws Exception {
     ...
     * 
     * @param className
     *            The name of the class to load (using '.' as the path
     *            separator)
     * @param resolveIt
     *            whether to resolve the class. (Necessary if an object is to be
     *            instantiated with the class e.g. using forName() or
     *            newInstance();
     * @return The class
    ...
    /**
     * Because the classloader allocates memory to load and store classes it
