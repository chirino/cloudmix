 ...
 * DATE          | Level  | Thread   | Category    | Class and line | Message
 * 
 */
public class DefaultLogParser implements LogParser {
	...
	/**
	 * Indicates if a given sequence may be successfully matched.
	 * Can be useful when reading multi-line entries from the log input stream.
	 * 
	...
	public boolean isPossibleMatch(CharSequence line) {
		...
		return s.contains("INFO") || s.contains("WARN") || s.contains("FATAL")
		       || s.contains("ERROR") || s.contains("DEBUG") || s.contains("TRACE");
	}
