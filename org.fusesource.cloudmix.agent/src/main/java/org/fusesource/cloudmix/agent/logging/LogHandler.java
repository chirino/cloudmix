...
/**
 * Facilitates the retrieval of and search for log records independently of the actual 
 * log format 
 *
 */
public class LogHandler {
	 ...
	 * @param category the category such as OsgiServiceFactoryBean,
	...
	public List<LogRecord> findRecords(String level, String category, int start, int max) {
		...
		for (int i = start; i < records.size() && recs.size() <= max; i++) {
        	LogRecord record = records.get(i);
        	if (level.equalsIgnoreCase(record.getLevel())
        		&& matches(record.getCategory(), category)) { 
        		recs.add(record);
        	}
        }
        return recs;
	...
	/**
	 * Reads the log file and converts the matching records into LogRecords
	 * Huge logs may be handled differently, for ex, by copying LogRecords into db tables
	 * or using NIO file channels, etc 
	 * @param r log reader
	 */
